...
def consultar_estado_upload(
    pfx_bytes: bytes, password: str,
    rut_consultante: str, rut_emisor: str, track_id: str,
    ambiente: str = "certificacion",
    token: str = None,
) -> dict:
    """Consulta el estado de un ENVÍO DTE por su Track ID (getEstUp, SOAP DTEWS).

    A diferencia de getEstDte (que consulta por datos del documento y da falsos
    negativos FAU en guías/boletas), este método pregunta directamente por el
    Track ID del envío — es el que refleja de forma confiable si el sobre fue
    aceptado o rechazado. Es el método adecuado para guías (52) y facturas (33/34).

    Returns:
        dict {ok, estado, glosa, aceptado, rechazado, respuesta_cruda} donde
        `estado` es el código del SII para el envío:
          EPR → Envío Procesado (aceptado; revisar detalle de reparos)
          SOK → Schema OK / recibido, en proceso
          RCT → Rechazado por error de schema
          RFR → Rechazado por firma
          RSC → Rechazado por schema
          RCH → Rechazado
          FAU / RCO → aún en proceso o con observaciones
    """
    def _split(rut):
        rut = rut.replace(".", "").replace("-", "")
        return rut[:-1], rut[-1]
    ...
    # getEstUp vive en QueryEstUp.jws (no QueryEstDte.jws)
    url = DTEWS[ambiente]["query"].replace("QueryEstDte.jws", "QueryEstUp.jws")
    _host = "maullin" if ambiente != "produccion" else "palena"
    soap = (
        '<?xml version="1.0" encoding="UTF-8"?>'
        '<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" '
        'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '
        'xmlns:xsd="http://www.w3.org/2001/XMLSchema">'
        '<SOAP-ENV:Body>'
        f'<m:getEstUp xmlns:m="http://{_host}.sii.cl/DTEWS/QueryEstUp.jws">'
        f'<Rut xsi:type="xsd:string">{re_}</Rut>'
        f'<Dv xsi:type="xsd:string">{dvr_e}</Dv>'
    ...
    m_glosa = (re.search(r"<GLOSA(?:_ESTADO)?>([^<]+)</GLOSA(?:_ESTADO)?>", texto) or
               re.search(r"&lt;GLOSA(?:_ESTADO)?&gt;([^&]+)&lt;/GLOSA", texto))
    # Detalle de aceptados/rechazados/reparos si viene
    def _num(tag):
        mm = (re.search(r"<%s>([^<]+)</%s>" % (tag, tag), texto) or
              re.search(r"&lt;%s&gt;([^&]+)&lt;/%s&gt;" % (tag, tag), texto))
        try:
            return int(mm.group(1).strip()) if mm else None
        except Exception:
            return None

    estado = m_estado.group(1).strip() if m_estado else None
    glosa = m_glosa.group(1).strip() if m_glosa else None
    aceptados = _num("ACEPTADOS")
    rechazados = _num("RECHAZADOS")
    reparos = _num("REPAROS")

    ESTADOS_ACEPTA = ("EPR", "DOK")
    ESTADOS_RECHAZA = ("RCT", "RFR", "RSC", "RCH", "RDC")
    aceptado = estado in ESTADOS_ACEPTA and (rechazados in (0, None))
    rechazado = estado in ESTADOS_RECHAZA or (rechazados is not None and rechazados > 0)
